...
// .wrangler/tmp/bundle-OCRPQu/middleware-loader.entry.ts
init_modules_watch_stub();

// .wrangler/tmp/bundle-OCRPQu/middleware-insertion-facade.js
init_modules_watch_stub();
...
var Context = class {
  ...
  html = /* @__PURE__ */ __name((html4, arg, headers) => {
    const res = /* @__PURE__ */ __name((html22) => this.#newResponse(html22, arg, setDefaultContentType("text/html; charset=UTF-8", headers)), "res");
    return typeof html4 === "object" ? resolveCallback(html4, HtmlEscapedCallbackPhase.Stringify, false, {}).then(res) : res(html4);
  }, "html");
...
var html = /* @__PURE__ */ __name((strings, ...values) => {
  ...
  for (let i = 0, len = strings.length - 1; i < len; i++) {
    buffer[0] += strings[i];
    const children2 = Array.isArray(values[i]) ? values[i].flat(Infinity) : [values[i]];
...
// node_modules/hono/dist/jsx/dom/context.js
var createContextProviderFunction = /* @__PURE__ */ __name((values) => ({ value, children: children2 }) => {
  if (!children2) {
    return void 0;
  }
  const props = {
  ...
  if (Array.isArray(children2)) {
    props.children.push(...children2.flat());
  } else {
    props.children.push(children2);
  }
  props.children.push({
    tag: setInternalTagFlag(() => {
      values.pop();
...
init_modules_watch_stub();
var toArray = /* @__PURE__ */ __name((children2) => Array.isArray(children2) ? children2 : [children2], "toArray");
...
var returnWithoutSpecialBehavior = /* @__PURE__ */ __name((tag, children2, props) => raw(new JSXNode(tag, props, toArray(children2 ?? [])).toString()), "returnWithoutSpecialBehavior");
var documentMetadataTag = /* @__PURE__ */ __name((tag, children2, props, sort) => {
...
var title = /* @__PURE__ */ __name(({ children: children2, ...props }) => {
...
var link = /* @__PURE__ */ __name(({ children: children2, ...props }) => {
...
var meta = /* @__PURE__ */ __name(({ children: children2, ...props }) => {
  const nameSpaceContext3 = getNameSpaceContext();
...
var newJSXNode = /* @__PURE__ */ __name((tag, { children: children2, ...props }) => new JSXNode(tag, props, toArray(children2 ?? [])), "newJSXNode");
...
var childrenToStringToBuffer = /* @__PURE__ */ __name((children2, buffer) => {
...
var jsxFn = /* @__PURE__ */ __name((tag, props, children2) => {
  if (!initDomRenderer) {
...
var applyProps = /* @__PURE__ */ __name((container, attributes2, oldAttributes) => {
  attributes2 ||= {};
  ...
  if (oldAttributes) {
    for (let key in oldAttributes) {
      ...
      if (key !== "children" && !(key in attributes2)) {
        key = normalizeIntrinsicElementKey(key);
        const eventSpec = getEventSpec(key);
        if (eventSpec) {
...
var getNextChildren = /* @__PURE__ */ __name((node, container, nextChildren, childrenToRemove, callbacks) => {
  ...
  if (typeof node.tag === "function") {
    node[DOM_STASH][1][STASH_EFFECT]?.forEach((data2) => callbacks.push(data2));
  }
...
var removeNode = /* @__PURE__ */ __name((node) => {
  if (!isNodeString(node)) {
    node[DOM_STASH]?.[1][STASH_EFFECT]?.forEach((data2) => data2[2]?.());
...
var applyNodeObject = /* @__PURE__ */ __name((node, container, isNew) => {
  ...
  const remove2 = [];
  const callbacks = [];
  getNextChildren(node, container, next2, remove2, callbacks);
  remove2.forEach(removeNode);
  ...
  if (isNew) {
    ...
    if (offsetByNextNode !== void 0) {
      ...
      offset = findChildNodeIndex(childNodes, next2.find((n) => n.tag !== HONO_PORTAL_ELEMENT && n.e)?.e) ?? -1;
    }
    if (offset === -1) {
      isNew = true;
    }
  }
  for (let i = 0, len = next2.length; i < len; i++, offset++) {
    const child = next2[i];
    let el;
    if (child.s && child.e) {
      el = child.e;
...
var build = /* @__PURE__ */ __name((context, node, children2) => {
  const buildWithPreviousChildren = !children2 && node.pC;
  if (children2) {
    node.pC ||= node.vC;
  }
  let foundErrorHandler;
  try {
    children2 ||= typeof node.tag == "function" ? invokeTag(context, node) : toArray(node.props.children);
    if (children2[0]?.tag === "" && children2[0][DOM_ERROR_HANDLER]) {
      foundErrorHandler = children2[0][DOM_ERROR_HANDLER];
    ...
    for (let i = 0; i < children2.length; i++) {
      if (Array.isArray(children2[i])) {
        children2.splice(i, 1, ...children2[i].flat());
      }
      let child = buildNode(children2[i]);
      if (child) {
...
var createPortal = /* @__PURE__ */ __name((children2, container, key) => ({
  ...
  return {
    ...
    props: {
      value: {
        pending: data2 !== null,
        data: data2,
        method: data2 ? "post" : null,
        action: data2 ? action : null
      },
      children: {
        tag: "form",
        props: {
...
// node_modules/hono/dist/jsx/dom/components.js
var ErrorBoundary = /* @__PURE__ */ __name(({ children: children2, fallback, fallbackRender, onError }) => {
  const res = Fragment({ children: children2 });
  res[DOM_ERROR_HANDLER] = (err) => {
...
var Suspense2 = /* @__PURE__ */ __name(async ({
    ...
    if (e instanceof Promise) {
      resArray = [
        e.then(() => {
          stackNode[DOM_STASH][0] = 0;
          buildDataStack.push([[], stackNode]);
          return childrenToString(children2).then(popNodeStack);
        })
      ];
    } else {
      throw e;
  ...
  if (resArray.some((res) => res instanceof Promise)) {
    const index2 = suspenseCounter++;
    const fallbackStr = await fallback.toString();
    return raw(`<template id="H:${index2}"></template>${fallbackStr}<!--/$-->`, [
        ...
        return Promise.all(resArray).then(async (htmlArray) => {
          ...
          if (buffer) {
            buffer[0] = buffer[0].replace(
              new RegExp(`<template id="H:${index2}"></template>.*?<!--/\\$-->`),
              content
            );
          }
          let html4 = buffer ? "" : `<template data-hono-target="H:${index2}">${content}</template><script${nonce ? ` nonce="${nonce}"` : ""}>
((d,c,n) => {
c=d.currentScript.previousSibling
d=d.getElementById('H:${index2}')
if(!d)return
...
<\/script>`;
          ...
          if (phase === HtmlEscapedCallbackPhase.Stream) {
            html4 = await resolveCallback(html4, HtmlEscapedCallbackPhase.BeforeStream, true, context);
          }
          return raw(html4, callbacks);
        });
      }
    ]);
  } else {
...
var childrenToString = /* @__PURE__ */ __name(async (children2) => {
  try {
    return children2.flat().map((c) => c == null || typeof c === "boolean" ? "" : c.toString());
  } catch (e) {
    if (e instanceof Promise) {
      await e;
      return childrenToString(children2);
    } else {
      throw e;
    }
  }
}, "childrenToString");
var ErrorBoundary2 = /* @__PURE__ */ __name(async ({ children: children2, fallback, fallbackRender, onError }) => {
  if (!children2) {
    return raw("");
  }
  if (!Array.isArray(children2)) {
  ...
  try {
    ...
    if (e instanceof Promise) {
      resArray = [
        e.then(() => childrenToString(children2)).catch((e2) => fallbackRes(e2))
      ];
    } else {
  ...
  if (resArray.some((res) => res instanceof Promise)) {
    fallbackStr ||= await fallback?.toString();
    const index2 = errorBoundaryCounter++;
    const replaceRe = RegExp(`(<template id="E:${index2}"></template>.*?)(.*?)(<!--E:${index2}-->)`);
    const caught = false;
    const catchCallback = /* @__PURE__ */ __name(({ error: error2, buffer }) => {
      ...
      return buffer ? "" : `<template data-hono-target="E:${index2}">${fallbackResString}</template><script>
((d,c,n) => {
c=d.currentScript.previousSibling
d=d.getElementById('E:${index2}')
if(!d)return
do{n=d.nextSibling;n.remove()}while(n.nodeType!=8||n.nodeValue!='E:${index2}')
d.replaceWith(c.content)
})(document)
<\/script>`;
    }, "catchCallback");
    ...
    return raw(`<template id="E:${index2}"></template><!--E:${index2}-->`, [
      ({ phase, buffer, context }) => {
        if (phase === HtmlEscapedCallbackPhase.BeforeStream) {
          return;
        }
        return promiseAll.then(async (htmlArray) => {
          ...
          htmlArray = htmlArray.flat();
          const content = htmlArray.join("");
          let html4 = buffer ? "" : `<template data-hono-target="E:${index2}">${content}</template><script${nonce ? ` nonce="${nonce}"` : ""}>
((d,c) => {
c=d.currentScript.previousSibling
d=d.getElementById('E:${index2}')
if(!d)return
...
if(!d)return
n=d.nextSibling
while(n.nodeType!=8||n.nodeValue!='E:${index2}'){n=n.nextSibling}
n.remove()
d.remove()
})(document)
<\/script>`),
...
function jsxDEV2(tag, props, key) {
  ...
  if (!props || !("children" in props)) {
    ...
    node = Array.isArray(children2) ? jsxFn(tag, props, children2) : jsxFn(tag, props, [children2]);
  }
  node.key = key;
  return node;
}
...
// src/index.js
var app = new Hono2();
var Layout = /* @__PURE__ */ __name((props) => html`<!DOCTYPE html>
  <html lang="ja">
    <head>
      <meta charset="UTF-8" />
...
var IndexPage = /* @__PURE__ */ __name(() => /* @__PURE__ */ jsxDEV2(Layout, { title: "\u96E8\u91CF\u5224\u5B9A\u7D50\u679C", children: [
  /* @__PURE__ */ jsxDEV2("div", { className: "container", children: [
    /* @__PURE__ */ jsxDEV2("h1", { children: "\u96E8\u91CF\u5224\u5B9A\u7D50\u679C" }),
    /* @__PURE__ */ jsxDEV2("div", { className: "form-container", children: [
      /* @__PURE__ */ jsxDEV2("label", { htmlFor: "location-picker", children: "\u89B3\u6E2C\u5730:" }),
      /* @__PURE__ */ jsxDEV2("select", { id: "location-picker" }),
      /* @__PURE__ */ jsxDEV2("label", { htmlFor: "date-picker", children: "\u57FA\u6E96\u65E5:" }),
      /* @__PURE__ */ jsxDEV2("input", { type: "date", id: "date-picker" })
    ] }),
    /* @__PURE__ */ jsxDEV2("div", { id: "results", children: [
      /* @__PURE__ */ jsxDEV2("div", { className: "info-box", children: [
        /* @__PURE__ */ jsxDEV2("h2", { id: "location", children: "\u5834\u6240: ..." }),
        /* @__PURE__ */ jsxDEV2("p", { children: [
          "\u57FA\u6E96\u65E5: ",
          /* @__PURE__ */ jsxDEV2("strong", { id: "base-date", children: "..." })
        ] }),
        /* @__PURE__ */ jsxDEV2("p", { children: [
          "\u57FA\u6E96\u65E5\u307E\u3067\u306E3\u65E5\u9593\u306E\u5408\u8A08\u96E8\u91CF: ",
          /* @__PURE__ */ jsxDEV2("strong", { id: "total-3-days", children: "... mm" })
        ] }),
        /* @__PURE__ */ jsxDEV2("p", { children: [
          "\u57FA\u6E96\u65E5\u307E\u3067\u306E30\u65E5\u9593\u306E\u5408\u8A08\u96E8\u91CF: ",
          /* @__PURE__ */ jsxDEV2("strong", { id: "total-30-days", children: "... mm" })
        ] })
      ] }),
      /* @__PURE__ */ jsxDEV2("div", { className: "chart-container", children: [
        /* @__PURE__ */ jsxDEV2("h2", { children: "\u904E\u53BB30\u65E5\u9593\u306E\u964D\u6C34\u91CF\u30B0\u30E9\u30D5" }),
        /* @__PURE__ */ jsxDEV2("canvas", { id: "precipitationChart" })
      ] }),
      /* @__PURE__ */ jsxDEV2("h2", { children: "\u904E\u53BB30\u65E5\u9593\u306E\u964D\u6C34\u91CF\u30AB\u30EC\u30F3\u30C0\u30FC" }),
      /* @__PURE__ */ jsxDEV2("div", { id: "calendar-container" })
    ] })
  ] }),
  /* @__PURE__ */ jsxDEV2("script", { children: html`
        let chart;
...
var MultiPage = /* @__PURE__ */ __name(() => /* @__PURE__ */ jsxDEV2(Layout, { title: "4\u5730\u70B9\u540C\u6642\u8868\u793A - \u96E8\u91CF\u5224\u5B9A\u7D50\u679C", children: [
  /* @__PURE__ */ jsxDEV2("div", { className: "container multi-container", children: [
    /* @__PURE__ */ jsxDEV2("h1", { children: "4\u5730\u70B9\u540C\u6642\u8868\u793A - \u96E8\u91CF\u5224\u5B9A\u7D50\u679C" }),
    /* @__PURE__ */ jsxDEV2("div", { style: "margin-bottom:1.5em; display: flex; align-items: center; gap: 1.5em; flex-wrap: wrap;", children: [
      /* @__PURE__ */ jsxDEV2("label", { htmlFor: "date-picker", children: "\u57FA\u6E96\u65E5:" }),
      /* @__PURE__ */ jsxDEV2("input", { type: "date", id: "date-picker" }),
      /* @__PURE__ */ jsxDEV2("div", { id: "location-pickers", style: "display: flex; gap: 1em; flex-wrap: wrap;" })
    ] }),
    /* @__PURE__ */ jsxDEV2("div", { className: "four-columns", id: "panels-container" })
  ] }),
  /* @__PURE__ */ jsxDEV2("script", { children: html`
          let locationsData = {};
          let allBlockNos = [];
...
var URL_TEMPLATE = "https://www.data.jma.go.jp/stats/etrn/view/daily_{url_type}.php?prec_no={prec_no}&block_no={block_no}&year={year}&month={month}&day=&view=";
async function getPrecipitationData(year, month, prec_no, block_no, url_type) {
  const url = URL_TEMPLATE.replace("{year}", year).replace("{month}", month).replace("{prec_no}", prec_no).replace("{block_no}", block_no).replace("{url_type}", url_type);
  try {
    ...
    const html4 = decoder.decode(buffer);
    return html4;
  } catch (e) {
...
__name(getPrecipitationData, "getPrecipitationData");
function parseData(html_content, url_type) {
  const $2 = load(html_content);
  const table = $2("#tablefix1");
  if (!table.length) return {};
  const precip_col_index = url_type === "s1" ? 3 : 1;
  const data2 = {};
  table.find("tr").slice(4).each((i, row) => {
    const cols = $2(row).find("td");
    if (cols.length > precip_col_index && /^\d+$/.test($2(cols[0]).text().trim())) {
      try {
        const day = parseInt($2(cols[0]).text().trim(), 10);
        const precipitation_text = $2(cols[precip_col_index]).text().trim().replace("--", "0").replace(")", "").replace("]", "");
        const precipitation = precipitation_text && precipitation_text !== "" ? parseFloat(precipitation_text) : 0;
        if (!isNaN(precipitation)) {
          data2[day] = precipitation;
        }
      } catch (e) {
      }
    }
...
app.get("/", (c) => c.html(/* @__PURE__ */ jsxDEV2(IndexPage, {})));
app.get("/multi", (c) => c.html(/* @__PURE__ */ jsxDEV2(MultiPage, {})));
app.get("/api/precipitation", async (c) => {
  ...
  let base_date;
  try {
    base_date = /* @__PURE__ */ new Date(date + "T00:00:00");
    if (isNaN(base_date.getTime())) throw new Error("Invalid date");
  } catch (e) {
    base_date = /* @__PURE__ */ new Date();
    base_date.setDate(base_date.getDate() - 1);
  }
  const monthly_data_cache = /* @__PURE__ */ new Map();
  const get_month_data = /* @__PURE__ */ __name(async (year, month) => {
  ...
  for (let i = 0; i < 30; i++) {
    ...
    precip_list.push(month_data[day] || 0);
